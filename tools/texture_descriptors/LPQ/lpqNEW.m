function LPQdesc = lpqNEW(img,winSize,decorr,freqestim,mode)
...
if decorr == 1
    ...
    [U,S,V]=svd(A*D*A);
   % "sign convention", e.g., force
    % the sign of the largest magnitude element  to be
    % positive by possibly multiplying by -1
    for i=1:size(V,2)
        Vcol=V(:,i);
        [a,b]=(max(abs(Vcol)));
        if sign(Vcol(b))==-1
